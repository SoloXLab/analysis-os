...
- 2026-07-03：v1.2 — 发现 v1.1 的所有规则在实际执行中被静默跳过（规则写在技能文件里 ≠ 规则被对照检查过），新增 `checklist.md` 强制合规闸门，作为草稿完成后、发送前的独立核查步骤，且要求向用户可见地披露哪些要求被降级/跳过，而不只是内部核查。详见 `CHANGELOG.md` [1.2.0]。
